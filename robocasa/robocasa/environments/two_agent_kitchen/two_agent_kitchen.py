...
class TwoAgentKitchen(ManipulationEnv):
    ...
    def __init__(
        ...
        # intialize cameras for two agents
        self._cam_configs = deepcopy(KU.CAM_CONFIGS)
        robot1_frontview_config = dict( # add extra camera for robot1 frontview, can also add more cameras
            pos=[-0.50, 0, 0.95],
        ...
        # two PandaMobiles by default, ['PandaMobile', 'PandaMobile']
        assert isinstance(robots, list), 'only list of robots is supported'
        assert len(robots) == 2, 'only two agents are supported'
        
        if robots[0] == "PandaMobile" and robots[1] == "PandaMobile":
    ...
    def _load_model(self):
        ...
        # set robot positions ----------------------------------------------------------------------------------------------------- modified
        if self.init_robot_base_pos is not None:
            assert isinstance(self.init_robot_base_pos, list)
            
            ref_fixture0 = self.get_fixture(self.init_robot_base_pos[0])
            robot_base_pos0, robot_base_ori0 = self.compute_robot_base_placement_pose(ref_fixture=ref_fixture0)
            robot_model0 = self.robots[0].robot_model
            robot_model0.set_base_xpos(robot_base_pos0)
            robot_model0.set_base_ori(robot_base_ori0)
            
            ref_fixture1 = self.get_fixture(self.init_robot_base_pos[1])
            robot_base_pos1, robot_base_ori1 = self.compute_robot_base_placement_pose(ref_fixture=ref_fixture1)
            robot_model1 = self.robots[1].robot_model
            robot_model1.set_base_xpos(robot_base_pos1)
            robot_model1.set_base_ori(robot_base_ori1)
            ...
            while True:
                ref_fixture0 = self.rng.choice(fixtures)
                fxtr_class0 = type(ref_fixture0).__name__
                if fxtr_class0 not in valid_src_fixture_classes:
                    continue
                break
            ...
            while True:
                ref_fixture1 = self.rng.choice(fixtures)
                fxtr_class1 = type(ref_fixture1).__name__
                if fxtr_class1 not in valid_src_fixture_classes or fxtr_class1 == fxtr_class0: # make sure two robots are not placed at the same fixture
                    continue
                break
    ...
    def _reset_internal(self):
        ...
        action = np.zeros(24) # apply empty action, only apply for PandaMobile or VX300SMobile ------------------------------------ modified
    ...
    def _setup_observables(self):
        ...
        def world_pose_in_gripper0(obs_cache):
            return T.pose_inv(T.pose2mat((obs_cache[f"{pf0}eef_pos"], obs_cache[f"{pf0}eef_quat"]))) if\
                f"{pf0}eef_pos" in obs_cache and f"{pf0}eef_quat" in obs_cache else np.eye(4)
        
        @sensor(modality=modality)
        def world_pose_in_gripper1(obs_cache):
            return T.pose_inv(T.pose2mat((obs_cache[f"{pf1}eef_pos"], obs_cache[f"{pf1}eef_quat"]))) if\
                f"{pf1}eef_pos" in obs_cache and f"{pf1}eef_quat" in obs_cache else np.eye(4)
        
        sensors = [world_pose_in_gripper0] + [world_pose_in_gripper1]
        names = ["world_pose_in_gripper0"] + ["world_pose_in_gripper1"]
        actives = [False] + [False]
    ...
    def _create_obj_sensors(self, obj_name, modality="object"):
        ...
        def obj_to_eef0_pos(obs_cache):            
            ...
            obs_cache[f"{obj_name}_to_{pf0}eef_quat"] = rel_quat
            return rel_pos

        @sensor(modality=modality)
        def obj_to_eef0_quat(obs_cache):
            return obs_cache[f"{obj_name}_to_{pf0}eef_quat"] if \
                f"{obj_name}_to_{pf0}eef_quat" in obs_cache else np.zeros(4)
        
        @sensor(modality=modality)
        def obj_to_eef1_pos(obs_cache):            
            # Immediately return default value if cache is empty
            if any([name not in obs_cache for name in
                    [f"{obj_name}_pos", f"{obj_name}_quat", "world_pose_in_gripper"]]):
                return np.zeros(3)
            ...
            rel_pose = T.pose_in_A_to_pose_in_B(obj_pose, obs_cache["world_pose_in_gripper"])
            rel_pos, rel_quat = T.mat2pose(rel_pose)
            obs_cache[f"{obj_name}_to_{pf1}eef_quat"] = rel_quat
            return rel_pos

        @sensor(modality=modality)
        def obj_to_eef1_quat(obs_cache):
            return obs_cache[f"{obj_name}_to_{pf1}eef_quat"] if \
                f"{obj_name}_to_{pf1}eef_quat" in obs_cache else np.zeros(4)

        sensors = [obj_pos, obj_quat, obj_to_eef0_pos, obj_to_eef0_quat] + [obj_to_eef1_pos, obj_to_eef1_quat]
        names = [f"{obj_name}_pos", f"{obj_name}_quat", f"{obj_name}_to_{pf0}eef_pos", f"{obj_name}_to_{pf0}eef_quat"] \
            + [f"{obj_name}_to_{pf1}eef_pos", f"{obj_name}_to_{pf1}eef_quat"]
    ...
    def convert_rel_to_abs_action(self, rel_action): # ---------------------------------------------------------------------------- modified
        ...
        action_abs = np.zeros_like(rel_action)
        split_point = rel_action.shape[0] // 2
        rel_action0 = rel_action[:split_point]
        rel_action1 = rel_action[split_point:]
        
        robot0 = self.robots[0]
        robot0.control(rel_action0, policy_step=True)
        ac_pos0 = robot0.composite_controller.controllers["right"].goal_origin_to_eef_pos
        ac_ori0 = robot0.composite_controller.controllers["right"].goal_origin_to_eef_ori
        ac_ori0 = Rotation.from_matrix(ac_ori0).as_rotvec()
        action_abs[split_point:] = np.hstack([
            ac_pos0,
            ac_ori0,
            rel_action0[6:],
        ])
        ...
        robot1.control(rel_action1, policy_step=True)
        ac_pos1 = robot1.composite_controller.controllers["right"].goal_origin_to_eef_pos
        ac_ori1 = robot1.composite_controller.controllers["right"].goal_origin_to_eef_ori
        ac_ori1 = Rotation.from_matrix(ac_ori1).as_rotvec()
        action_abs[:split_point] = np.hstack([
            ac_pos1,
            ac_ori1,
            rel_action1[6:],
    ...
    def visualize(self, vis_settings):
        ...
        # Make multi robots semi-transparent -------------------------------------------------------------------------------------- modified
        
        robot_model0 = self.robots[0].robot_model
        visual_geom_names0 = robot_model0.visual_geoms
        for name in visual_geom_names0:
        ...
        robot_model1 = self.robots[1].robot_model
        visual_geom_names1 = robot_model1.visual_geoms
        for name in visual_geom_names1:
            rgba = self.sim.model.geom_rgba[self.sim.model.geom_name2id(name)]
            if self.translucent_robot:
                rgba[-1] = 0.20
            else:
