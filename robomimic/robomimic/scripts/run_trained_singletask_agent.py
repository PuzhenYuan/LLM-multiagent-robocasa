...
def run_rollout_singletask_policy(
    ...
    ob_dict = env.reset()
    if env_lang == None:
        env_lang = env._ep_lang_str
    policy.start_episode(lang=env_lang)
...
def run_trained_singletask_agent(args):
    ...
    if args.env_lang is not None:
        env_lang = args.env_lang
    else:
        env_lang = None # "task0_lang"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--env_lang",
        type=str,
        default="pick the vegetable from the counter", # None by default, should be like "task0_lang"
        help="(optional) set seed for rollouts",
    )
